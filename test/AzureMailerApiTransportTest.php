...
class AzureMailerTransportFactoryTest extends TestCase
{
    /**
     * @dataProvider getTransportData
     */
    public function testToString(AzureMailerApiTransport $transport, string $expected)
    {
    ...
    public static function getTransportData()
    {
        return [
            [
                new AzureMailerApiTransport('ENDPOINT', 'KEY', 'API_VERSION', 'ENGAGEMENT_TRACKING'),
                'azure+api://my-acs-resource.communication.azure.com',
            ],
            [
                (new AzureMailerApiTransport('ENDPOINT', 'KEY', 'API_VERSION', 'ENGAGEMENT_TRACKING'))->setHost('my-acs-resource.communication.azure.com'),
                'azure+api://my-acs-resource.communication.azure.com',
            ],
            [
                (new AzureMailerApiTransport('ENDPOINT', 'KEY', 'API_VERSION', 'ENGAGEMENT_TRACKING'))->setHost('my-acs-resource.communication.azure.com')->setPort(99),
                'azure+api://my-acs-resource.communication.azure.com:99',
            ],
        ];
    }
    ...
    public function testSend()
    {
        ...
        $httpClient
            ->expects($this->once())
            ->method('request')
            ->with('POST', 'https://my-acs-resource.communication.azure.com/emails:send?api-version=2023-03-31', [
                'body' => [
                    'personalizations' => [
                        [
                            'to' => [[
        ...
        $mailer = new AzureMailerApiTransport('foo', $httpClient);
        $mailer->send($email);
    }
    ...
    public function testLineBreaksInEncodedAttachment()
    {
        ...
        $mailer = new AzureMailerApiTransport('ENDPOINT', 'KEY', 'API_VERSION', 'ENGAGEMENT_TRACKING', $httpClient);
    ...
    public function testCustomHeader()
    {
        ...
        $transport = new AzureMailerApiTransport('ENDPOINT', 'KEY', 'API_VERSION', 'ENGAGEMENT_TRACKING');
        $method = new \ReflectionMethod(AzureMailerApiTransport::class, 'getPayload');
